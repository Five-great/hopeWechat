...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.Base.getData({flag:'xcsp',id: 0,pageindex: this.data.pageIndex,pagesize: this.data.pageSize}).then(res=>{
      // console.log(res)
       this.data.pageTotal = res.total;
       this.data.pageIndex++;
       res.state=="ok"?this.setData({xcspList: res.info}) : ''
    })
  },
  getXcspData(){
   let that = this;
   if( (that.data.pageIndex-1)*that.data.pageSize<=that.data.pageTotal){
        wx.Base.getData({flag:'xcsp',id: 0,pageindex: that.data.pageIndex,pagesize: that.data.pageSize}).then(res=>{
          that.data.pageIndex++;
          that.data.xcspList.push(...res.info)
          res.state=="ok"?that.setData({xcspList: that.data.xcspList,upLoadShow: 1}) : ''
        })
  ...
  onReady: function () {
   ...
   wxWOWCatchTap: function(e){
    //  console.log(e)
     setTimeout(()=>{
      e.target.dataset.idx?wx.navigateTo({url: '/pages/videoDetails/videoDetails?idx='+e.target.dataset.idx+'&url='+e.target.dataset.url+'&photo='+e.target.dataset.photo}):''
     },600)
  ...
  touchEnd(e){
   ...
   touchcheck(){
    ...
    that.data.upLoadShow == 0? that.getXcspData() : that.data.upLoadShow !=-1 ? that.setData({upLoadShow: 1}) : '';
   },
  /**
   * 生命周期函数--监听页面显示
   */
